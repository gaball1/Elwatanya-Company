...
/**
 * Mirrors distributeComponent for non-analyzed final items.
 * - Blocks analyzed items (must distribute per component) → ITEM_IS_ANALYZED.
 * - Validates sum === item.quantity and each qty > 0.
 * - Upserts contractor BOQ items with componentId === null.
 * - Derives item status / remaining from allocations (syncFinalItemState).
 */
export class DistributeItemUseCase {
  constructor(
  ...
  async execute(input: DistributeItemInput, user?: OwnershipActor, userId?: string): Promise<Result<FinalBoqItemResult>> {
    ...
    if (item.isAnalyzed && item.components.length > 0) {
      return Result.fail(
        new FinalBoqApplicationError(
          FinalBoqErrorCode.ITEM_IS_ANALYZED,
          'هذا البند متحلل، لا يمكن توزيعه مباشرة. يرجى توزيع المكونات.',
        ),
      );
    }
    ...
    const alreadyAllocated = allBoqs.reduce(
      (sum, b) =>
        sum +
        b.items
          .filter((i) => i.itemCode === input.itemCode && (i.componentId?.toValue() ?? null) === null)
    ...
    await this.prisma.$transaction(async (tx) => {
      for (const d of input.distribution) {
        ...
        const existingItem = existingStates.find(
          (i) => i.itemCode === input.itemCode && i.componentId === null,
        );
        ...
        const newItem: ContractorItemState = {
          ...
          totalValue: calcTotal(d.quantity, item.unitPrice),
          componentId: null,
        ...
        if (existingItem) {
          next = existingStates.map((i) =>
            i.itemCode === input.itemCode && i.componentId === null
              ? {
                  ...i,
                  assignedQuantity: i.assignedQuantity + d.quantity,
                  quantity: i.assignedQuantity + d.quantity,
                  totalValue: calcTotal(i.assignedQuantity + d.quantity, item.unitPrice),
                }
              : i,
          );
        } else {
    ...
    if (userId) {
      this.audit.log({ userId, entity: 'distribution', entityId: input.itemCode, action: 'DISTRIBUTE_ITEM', before: null, after: { itemCode: input.itemCode, distribution: input.distribution } });
    }
