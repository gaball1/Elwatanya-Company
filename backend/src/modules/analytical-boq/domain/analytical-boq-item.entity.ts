...
export interface AnalyticalBoqItemProps {
...
export class AnalyticalBoqItem extends AggregateRoot {
  private props: AnalyticalBoqItemProps;

  private constructor(
    props: AnalyticalBoqItemProps,
    id?: UniqueEntityId,
    createdAt?: Date,
    updatedAt?: Date,
  ) {
  ...
  public static create(input: {
    ...
    quantity: number;
    unitPrice: number;
  }): Result<AnalyticalBoqItem> {
    const validation = AnalyticalBoqItem.validateFields(input);
    ...
    return Result.ok(
      new AnalyticalBoqItem({
        ...
        totalValue: AnalyticalBoqItem.calculateTotalValue(values.quantity, values.unitPrice),
      }),
    );
  }

  public static createFromEmployer(input: {
    buildingId: UniqueEntityId;
    itemCode: string;
    description: string;
    unit: string;
    ...
    return AnalyticalBoqItem.create(input);
  }
  ...
  public static reconstitute(
    ...
    return new AnalyticalBoqItem(props, id, createdAt, updatedAt);
  }
  ...
  public update(input: {
    ...
    const validation = AnalyticalBoqItem.validateFields({
    ...
    this.props.totalValue = AnalyticalBoqItem.calculateTotalValue(values.quantity, values.unitPrice);
    return Result.ok();
  }

  public replaceFromEmployer(input: {
    description: string;
    unit: string;
    quantity: number;
    unitPrice: number;
