...
export class RejectApprovalUseCase {
  ...
  async execute(id: string, approvedBy: string, comment?: string): Promise<Result<Approval>> {
    try {
      ...
      if (approval.status !== 'pending') return Result.fail(new Error('Only pending requests can be rejected'));
      const updated = await this.repo.update(id, { status: 'rejected', approvedBy, comment: comment ?? '', approvedAt: new Date() });
